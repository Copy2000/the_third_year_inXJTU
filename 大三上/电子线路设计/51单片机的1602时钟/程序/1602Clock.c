...
code uint LED_CODE[] = {0x3F,0x06,0x5B,0x4F,0x66,0x6D,0x7D,0x07,
...
unsigned char now_shi=19,now_fen=20,now_miao;//运行时的时间变量
unsigned char temp_shi,temp_fen,temp_miao;//调时间的时间变量
unsigned char nao_shi=19,nao_fen=30,nao_miao;//闹钟的时间变量
...
void digital_number(int digital_num);
void main()
{	
	int digital_num=0;
	lcd_int ();	   //初始化lcd
	...
	while(1){
			...
			keyscan(digital_num);			   //调用按键开关控制函数
			if(now_shi==nao_shi&&now_fen==nao_fen&&now_miao==nao_miao)
			{	
				PlayMusic();//如果正在运行的时间为闹钟的时间，则闹铃响（此为音乐）			
			}	  
	}	
}
...
void keyscan(int digital_num)					  //按键控制，k1调时或分，k2加时或分，k3减时或分，k4确定调好的时间，k5调闹钟
{
	if(K1==0)
	{
		delay1(1000);
	...
	if(K4==0)
	{
		...
		display_mode();			   //调用显示模式函数
		while(digital_num<10)
		{
			delay1(1000000);
		digital_number(digital_num); //如果调整，则数码管就动
			digital_num++;
	}}
